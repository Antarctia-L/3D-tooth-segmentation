...
class PointMLP(nn.Module):
    ...
    def __init__(self, num_classes=50,points=10000, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        # encoder
        self.cic11 = CIC(npoint=10000, radius=0.2, k=k, in_channels=additional_channel, output_channels=64, bottleneck_ratio=2, curve_config=curve_config[setting][0])
        self.cic12 = CIC(npoint=10000, radius=0.2, k=k, in_channels=64, output_channels=64, bottleneck_ratio=4, curve_config=curve_config[setting][0])
        ...
        self.up_cic2 = CIC(npoint=10000, radius=0.2, k=k, in_channels=128+64+64+3, output_channels=256, bottleneck_ratio=4)
        self.up_cic1 = CIC(npoint=10000, radius=0.2, k=k, in_channels=256, output_channels=256, bottleneck_ratio=4)
...
def pointMLP_seg(num_classes=17, num_channels=15, **kwargs) -> PointMLP:
    return PointMLP(num_classes=num_classes, points=10000, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
                 gmp_dim=64,cls_dim=64, num_channels=num_channels,
                 num_channels2=15, k2=32, emb_dims2=1024, dropout2=0.5, k=32, setting='default', **kwargs)


if __name__ == '__main__':
    data = torch.rand(2, 15, 10000)

...
class PointMLP(nn.Module):
    ...
    def __init__(self, num_classes=17,points=8000, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        # encoder
        self.cic11 = CIC(npoint=8000, radius=0.2, k=k, in_channels=additional_channel, output_channels=64, bottleneck_ratio=2, curve_config=curve_config[setting][0])
        self.cic12 = CIC(npoint=8000, radius=0.2, k=k, in_channels=64, output_channels=64, bottleneck_ratio=4, curve_config=curve_config[setting][0])
        ...
        self.up_cic2 = CIC(npoint=8000, radius=0.2, k=k, in_channels=128+64+64+3, output_channels=256, bottleneck_ratio=4)
        self.up_cic1 = CIC(npoint=8000, radius=0.2, k=k, in_channels=256, output_channels=256, bottleneck_ratio=4)
    ...
    def forward(self, x1, l=None):
        ...
        x = torch.cat((x1[:, 9:12, :], x1[:, 21:24, :]), dim=1)
...
def pointMLP_seg(num_classes=17, num_channels=24, **kwargs) -> PointMLP:
    return PointMLP(num_classes=num_classes, points=8000, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],

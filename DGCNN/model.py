...
class DC_Net(nn.Module):
    def __init__(self, args, output_channels=17):
        ...
        self.conv9 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1, bias=False),
                                   self.bn9,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv10 = nn.Sequential(nn.Conv2d(192, 1024, kernel_size=1, bias=False),
                                   self.bn10,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv11 = nn.Sequential(nn.Conv2d(704, 256, kernel_size=1, bias=False),
                                   self.bn11,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.dp1 = nn.Dropout(p=args.dropout)
        ...
        self.conv15 = nn.Sequential(nn.Conv1d(704, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn15,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x):
        ...
        x1 = self.max1(x)
        x2 = self.avg1(x)
        x1 = x1.transpose(1, 2)  # (batch_size, 32, num_points, k)
        x2 = x2.transpose(1, 2)  # (batch_size, 32, num_points, k)
        # print("shape x1", x1.shape)
        # print("shape x2", x2.shape)
        x3 = torch.cat((x1, x2), dim=1)  # (batch_size, 64, num_points)
        ...
        x = x.transpose(1, 2)
        x = self.max2(x)
        x = x.transpose(1, 2) # (batch_size, 1024, num_points) -> (batch_size, 512, emb_dims)
        # print("shape max", x.shape)
        x = torch.cat((x, x3, x4, x5), dim=1)   # (batch_size, 512+64*3, num_points)
        # print("shape torch", x.shape)
        x = x.unsqueeze(-1).expand(-1, -1, -1, k)
        x = self.conv11(x)  # (batch_size, 512+64*3, num_points) -> (batch_size, 256, num_points)
        x = self.dp1(x)

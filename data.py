...
#    return all_data, all_label
    if partition == 'train':
        h5_filename = os.path.join(DATA_DIR, 'data_2000.h5')
    else:
        h5_filename = os.path.join(DATA_DIR, 'data_2000.h5')
    with h5py.File(h5_filename, 'r') as f:
        
        keys = list(f.keys())
        data_keys = [k for k in keys if k.startswith('points_') or k.startswith('label_')]
        num_datasets = len(data_keys) // 2
        num_points = 2000
        # 
        data = np.empty((num_datasets, num_points, 3), dtype=np.float32)
        # label = np.empty((num_datasets, 10000, 1), dtype=np.int32)
        labels = np.zeros((num_datasets, num_points))
        
        for i in range(num_datasets):
            data_key = f'points_{i}'
            label_key = f'label_{i}'
            # data[i] = f[data_key][:]
            data[i] = np.squeeze(f[data_key][:, :3].transpose(), axis=-1)
            # label[i] = np.expand_dims(f[label_key][:], axis=-1)
            label = f[label_key]#[i]  
            label = np.reshape(label, (1, num_points))
            labels[i] = label
    return data, labels
